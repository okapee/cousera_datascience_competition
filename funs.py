...
def get_feature_matrix(sales, test, items, list_lags, date_block_threshold):
    ...
    for block_num in sales["date_block_num"].unique():
        ...
        cur_items = (
            sales.loc[sales["date_block_num"] == block_num, "item_id"]
            .append(pd.Series(cur_items_aux))
            .unique()
        )
    ...
    gb.columns = [col[0] if col[-1] == "" else col[-1] for col in gb.columns.values]
    all_data = pd.merge(
        all_data, gb, how="left", on=["shop_id", "date_block_num"]
    ).fillna(0)
    ...
    gb.columns = [col[0] if col[-1] == "" else col[-1] for col in gb.columns.values]
    all_data = pd.merge(
        all_data, gb, how="left", on=["item_id", "date_block_num"]
    ).fillna(0)
    ...
    fit_cols = [
        ...
        if col[-1] in [str(item) for item in shift_range]
    ]
    # We will drop these at fitting stage
    to_drop_cols = list(
        set(list(all_data.columns)) - (set(fit_cols) | set(index_cols))
    ) + ["date_block_num"]
